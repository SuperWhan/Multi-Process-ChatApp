...
int add_user(int idx, USER * user_list, int pid, char * user_id, int pipe_to_child, int pipe_to_parent)
{
  ...
  USER temp;
  temp.m_pid = pid;
  temp.m_fd_to_user = pipe_to_parent;
  temp.m_fd_to_server = pipe_to_child;
  ...
  // printf("111111111111");
    ...
    if (user > -1) {
      ...
      if (find_user_index(user_list, user_id) == -1) {
        add_user(empty_slot, user_list, pid, user_id, pipe_user_to_child[1], pipe_child_to_user[0]);

      }
      if (pipe(pipe_child_to_server) == -1 || pipe(pipe_server_to_child) == -1) {
        perror("Pipe Error");
      }

      printf("%s joined the chat, slot: %d, p_id: %d\n", user_id, empty_slot, getpid());
      print_prompt("admin");
      pid = fork();
    ...
    if (pid == -1) {
      ...
      if (strlen(child_buf) > 1){
        ...
        read(pipe_child_to_server[0], server_buf, MAX_MSG);
        // printf("%s\n", server_buf );
        if (strlen(server_buf) > 1) {
          // printf("111111111");
          printf("Parent: %s", server_buf);
          print_prompt("admin");
        }
    } // parent process
